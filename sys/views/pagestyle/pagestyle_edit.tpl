...
function check_input(form) {
	if(form.name.value.replace(/(^[\s]*)|([\s]*?$)/g, "") == ''){
		alert("{html_lang name=pagestyle_set_need_name}",function(){
   			$("input[name=name]").focus();
   		});
		return false;
	}
	if(form.en_name.value.replace(/(^[\s]*)|([\s]*?$)/g, "") == ''){
		alert("{html_lang name=pagestyle_set_need_en_name}",function(){
   			$("input[name=en_name]").focus();
   		});
		return false;
	}
	if(form.path.value.replace(/(^[\s]*)|([\s]*?$)/g, "") == ''){
		alert("{html_lang name=pagestyle_set_need_path}",function(){
...
<div class="main">
	<form id="form1" name="form1" method="post" class='curr_form'
		...
		<h2>
			{if $isEdit}
				{html_lang name='pagestyle_set_edit_style_title'}
			{else}
				{html_lang name='pagestyle_set_add_style_title'}
			{/if}
		</h2>
		<div class="form">
			<div>
				<label>{html_lang name='pagestyle_set_add_style_name'}</label> 
				<input name="name" type="text" value="{$name|escape}" autocomplete="off" maxlength="20" />
			</div>
			<div>
				<label>{html_lang name='pagestyle_set_add_style_en_name'}</label>
				<input name="en_name" type="text" value="{$en_name|escape}" autocomplete="off" maxlength="20" />
			</div>
			<div>
				<label>{html_lang name='pagestyle_set_add_style_path'}</label> 
